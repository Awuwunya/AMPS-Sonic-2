...
static const char InExtWav [] = { ".wav" };
static const char InExtAiff [] = { ".aiff" };
...

{
	...
	if (ArgNumber <= 0x01)
	{
		printf (" -> Arguments: ConvPCM.exe Input01%s Input02%s Input03%s etc...\n\n", InExtWav, InExtWav, InExtWav);
		printf ("    Simply drag and drop %s or %s files onto this program to convert\n", InExtWav, InExtAiff);
		printf ("    to %s\n\n", OutExt);
	...
	/*	if (strcmp (ExtName, InExt) != 0x00)
		...
		bool NonWave = TRUE;
		if (strcmp (ExtName, InExtWav) == 0x00)
		{
			NonWave = FALSE;
		}
		else if (strcmp (ExtName, InExtAiff) == 0x00)
		{
			NonWave = FALSE;
		}
