...
; ---------------------------------------------------------------------------
		...
		moveq	#$B4,d0			; YM address: Panning and LFO
...
dMuteDAC:
	StopZ80					; wait for Z80 to stop
		...
		lea	dZ80+PCM1_Sample.l,a1	; load addresses for PCM 1 sample to a1
	...
	endm
		...
		lea	SampleList(pc),a2	; load address for the stop sample data into a2
		lea	dZ80+PCM2_Sample.l,a1	; load addresses for PCM 2 sample to a1
...
.nodacsfx
		...
		moveq	#$B4+2,d0		; YM address: Panning and LFO (FM3/6)
...
.index =	MusicIndex-(MusOff*4)		; effin ASS
		lea	.index(pc),a4		; get music pointer table with an offset
...
.sampmode
		...
		moveq	#0,d7
		moveq	#(1<<cfbRun)|(1<<cfbRest),d2; prepare running tracker and channel rest flags
...
.fm
		...
		moveq	#$80,d0			; YM address: Release Rate Operator 1
		jsr	WriteChYM(pc)		; write to YM according to channel
		moveq	#$88,d0			; YM address: Release Rate Operator 3
		jsr	WriteChYM(pc)		; write to YM according to channel
		moveq	#$84,d0			; YM address: Release Rate Operator 2
		jsr	WriteChYM(pc)		; write to YM according to channel
		moveq	#$8C,d0			; YM address: Release Rate Operator 4
		jsr	WriteChYM(pc)		; write to YM according to channel
...
; Dual PCM does	moveq	#$80,d1			; FM6 acts as DAC
...
dMutePSG:
		lea	dPSG.l,a1		; load PSG data port address to a1
