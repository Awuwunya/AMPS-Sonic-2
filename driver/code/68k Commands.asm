...
.cunt = 	.comm-$80			; AS is ASS
		jmp	.cunt(pc,d5.w)		; jump to appropriate handler
; ===========================================================================
; ---------------------------------------------------------------------------
...
.fuck = 	.false-$80			; AS is ASS
		jmp	.fuck(pc,d5.w)		; jump to appropriate handler (false command)
; ===========================================================================
; ---------------------------------------------------------------------------
...
dcNoisePSG:
		...
		move.b	(a4)+,dPSG.l		; send command to PSG port
		rts

.psg3
		move.b	#ctPSG3,cType(a5)	; make PSG3 not act on behalf of PSG4
		move.b	#ctPSG4|$1F,dPSG.l	; send PSG4 mute command to PSG
...
dcYM:
		move.b	(a4)+,d3		; load YM address from tracker to d0
		move.b	(a4)+,d0		; get command value from tracker to d1
		btst	#cfbInt,(a5)		; is this channel overridden by SFX?
		bne.s	Return_dcSound		; if so, skip

		cmp.b	#$30,d3			; is this register 00-2F?
		blo.w	.pt1			; if so, write to part 1 always
...
; ---------------------------------------------------------------------------
		...
		lea	dZ80+PCM_Volume1.l,a1	; get Z80 volume table to a1
...
.volloop
	...
	endm					; this actually saves some cycles
		...
		move.b	d2,dZ80+PCM1_VolumeCur+1.l; set PCM1 volume as mute
		move.b	d2,dZ80+PCM2_VolumeCur+1.l; set PCM2 volume as mute
	startZ80
; ---------------------------------------------------------------------------
...
WriteReg	macro	reg
	if "reg"<>""
		...
		WriteReg ALLARGS		; get the next argument
	endif
    endm
...
dUpdateVoiceFM:
	dCALC_VOICE				; get address of the specific voice to a1
		...
		moveq	#$B0,d0			; YM command: Algorithm & FeedBack
		or.b	d2,d0			; add channel offset to register
		move.b	d0,(a3)+		; write register to buffer
...
.slot
	...
	if safe=1
		...
		moveq	#$B4,d0			; YM command: Panning & LFO
		or.b	d2,d0			; add channel offset to register
		move.b	d0,(a3)+		; write register to buffer
...
.psg
		...
		move.b	cStatPSG4(a1),dPSG.l	; update PSG4 status to PSG port
		bra.s	.exit
; ===========================================================================
; ---------------------------------------------------------------------------
